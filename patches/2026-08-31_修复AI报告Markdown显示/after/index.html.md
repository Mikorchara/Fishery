...
<body>
    ...
    <script>
        ...
        // ---- 离线 Markdown 兜底渲染（CDN 加载失败时避免 AI 报告无法显示）----
        function fallbackMarkdown(src) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (/^\s*\|.*\|\s*$/.test(r)) {
                    ...
                    for (var j = 0; j < cells.length; j++) {
                        if (!/^:?-+:?$/.test(cells[j])) tds += '<td>' + cells[j] + '</td>';
                    }
                    out.push('<tr>' + tds + '</tr>');
                } else {
            ...
            // 4) 列表
            s = s.replace(/(^[-*] .*(?:\n[-*] .*)*)/gm, function(m) {
                return '<ul>' + m.replace(/^[-*] (.*)$/gm, '<li>$1</li>').replace(/\n/g, '') + '</ul>';
            });
            // 5) 加粗 / 斜体 / 行内代码
            s = s.replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>');
            s = s.replace(/\*([^*\n]+)\*/g, '<em>$1</em>');
            ...
            // 6) 保护表格再统一换行
            var tables = [];
            s = s.replace(/<table>[\s\S]*?<\/table>/g, function(m) {
                tables.push(m);
                return '\u0000' + (tables.length - 1) + '\u0000';
            });
            s = s.replace(/\n/g, '<br>');
            s = s.replace(/\u0000(\d+)\u0000/g, function(m, i) { return tables[+i]; });
            return s;
        }

        // 优先用 CDN 的 marked，失败自动回退到内置渲染；全程 try/catch 兜底
        function renderMarkdown(text) {
            try {
                var md = window.marked || fallbackMarkdown;
